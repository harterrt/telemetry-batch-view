...
class CrossSectionalViewTest extends FlatSpec {
  ...
  def getExampleCrossSectional(
    ...
    new CrossSectional(
      ...
      , days_active_6_sun = 0
      , days_possible = 5
      , days_possible_0_mon = 1
      , days_possible_1_tue = 1
      , days_possible_2_wed = 0
      , days_possible_3_thu = 0
      , days_possible_4_fri = 1
      , days_possible_5_sat = 1
      , days_possible_6_sun = 1
      ...
      , os_version_cfgs = Some(1)
      , pages_count_avg = Some(2100.0 / 7)
      , pages_count_min = Some(100)
      , pages_count_max = Some(400)
      , plugins_count_avg = Some(12.0 / 7)
      , plugins_count_cfgs = Some(2)
      , plugins_count_mode = Some(2)
      , start_date_oldest = "2016-01-01"
      , start_date_newest = "2016-01-05"
      , ss_length_badTimer = Some(0)
      , ss_length_negative = Some(0)
      , ss_length_tooLong = Some(0)
      , prev_ss_id_repeats = Some(2)
      , prof_creation_date = Some("2016-04-21T00:00:00.000Z")
      , prof_ss_counter_min = Some(1)
      , prof_ss_counter_max = Some(3)
      , prof_ss_counter_cfgs = Some(3)
      , search_counts_total = Some(42)
      , search_default_cfgs = Some(2)
      , search_default_mode = Some("grep")
      , session_num_total = Some(3)
      , subsess_branches = Some(1)
  ...
  it must "summarize active_addons properly" in {
    val actual = new CrossSectional(getExampleLongitudinal(
      ...
      , active_addons = Some(Seq(
            Map(("one", getExampleActiveAddon()), ("two", getExampleActiveAddon(name = "Not An Ad Blocker")))
    ...
    val expected = getExampleCrossSectional(
        client_id = "a"
      , addon_count_foreign_avg = Some(6.0 / 7.0)
      ...
      , addon_names_list = Seq(Some("Name"), Some("Not An Ad Blocker"))
