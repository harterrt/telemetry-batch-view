...
case class Longitudinal (
  ...
  private def bothOrNone[A, B](pair: (Option[A], Option[B])): Option[(A, B)] = {
    for {first <- pair._1; second <- pair._2} yield (first, second)
  }
  ...
  def weightedMean(values: Option[Seq[Option[Long]]]): Option[Double] = {
    (for{v <- values; csl <- this.cleanSessionLength} yield (v, csl))
      .flatMap(collate)
      .map(_.flatMap(pair => bothOrNone(pair))) //remove value, weight pairs containing None
      .flatMap(aggregation.weightedMean)
  }

  def weightedMode[A](values: Option[Seq[A]]): Option[A] = {
    def validatePair[A](pair: (A, Option[Long])): Option[(A, Long)] = {
      pair._2.map(weight => (pair._1, weight))
    }

    (for{v <- values; csl <- this.cleanSessionLength} yield (v, csl))
      .flatMap(collate)
      .map(_.flatMap(pair => validatePair(pair)))
      .flatMap(aggregation.weightedMode)
  }
