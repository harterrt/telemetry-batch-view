...
case class ActiveAddons (
  ...
  , val Type: Option[String] // `type` is reserved, and the spark pattern matching requires the field be present
...
case class Longitudinal (
  ...
  def weightedMean(values: Option[Seq[Option[Long]]]): Option[Double] = {
    (values, this.session_length) match {
      case (Some(v), Some(sl)) => Some(aggregation.weightedMean(v, sl))
      case _ => None
    }
  }
  ...
  def hasAdBlocker() = {
    def isAdBlocker(oname: Option[String]) = {
      val adb_stubs = Seq("adblock", "ublock")
      oname.map(name => adb_stubs.foldLeft(false)((acc, stub) => acc || (name.toLowerCase contains stub)))
        .getOrElse(false)
    }
    this.addonNames.getOrElse(Seq[Option[String]]()).foldLeft(false)((acc, name) => acc || isAdBlocker(name))
  }
  ...
  def daysPossibleByDOW(dow: Int) = {
    // Note that joda time uses 1:7 to refer to Monday through Sunday
    this.rangeOfPossibleDates.filter(_.getDayOfWeek == dow).size
  }

  def countPingReason(value: String) = {
    this.reason.getOrElse(Seq()).filter(_ == value).size
  }

  def previous_subsession_id_counts() = {
      this.previous_subsession_id.map(_.groupBy(identity).mapValues(_.size))
  }

  def ss_start_to_submission(): Option[Seq[Long]] = {
    (this.subsession_start_date, this.submission_date) match {
      case (Some(sd), Some(ssd)) => Some((sd zip ssd).map(x => get_date_diff(x._1, x._2)))
      case _ => None
    }
  }

  private def get_date_diff(begin: String, end: String) = {
    Days.daysBetween(parse_date(begin), parse_date(end)).getDays.toLong
  }

  private def parse_date(sdate: String) = {
...
case class CrossSectional (
  ...
  def this(base: Longitudinal) = {
    this(
      ...
      , days_active_3_thu = base.activeDaysByDOW(4)
      ...
      , days_possible_3_thu = base.daysPossibleByDOW(4)
      , days_possible_4_fri = base.daysPossibleByDOW(5)
      ...
      , prev_ss_id_repeats = base.previous_subsession_id_counts.map(_.values.max)
      ...
      , subsess_branches = base.previous_subsession_id_counts.map(_.values.filter(_ > 1).size)
      , date_skew_per_ping_avg = base.ss_start_to_submission.map(aggregation.mean).getOrElse(None)
      , date_skew_per_ping_max = base.ss_start_to_submission.map(_.max)
      , date_skew_per_ping_min = base.ss_start_to_submission.map(_.min)
    )
  }
}
